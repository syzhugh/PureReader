...
public class ZhiHuFragment extends BaseFragment {
    // birthday of ZhiHu is 2013.5.20
//    DouBanProtocol mDouBanProtocol;
    ZhiHuProtocol mZhiHuProtocol;
    List<StoriesEntity> mStoriesEntities;
    ...
    protected void handleMsgByChild(Message msg) {
        if (mStoriesEntities == null) {
            ...
            mAdapter = new ZhiHuAdapter(mStoriesEntities);
            mParentRecyclerView.setAdapter(mAdapter);
        }
        if (addDatasType == 1 || addDatasType == 3) {
            mStoriesEntities.clear();
            mStoriesEntities.addAll((List<StoriesEntity>) msg.obj);
            mAdapter.notifyDataSetChanged();
        }
        if (addDatasType == 2) {
            int position = mStoriesEntities.size();
            mStoriesEntities.addAll((List<StoriesEntity>) msg.obj);
            mAdapter.notifyItemRangeInserted(position, ((List<StoriesEntity>) msg.obj).size());
        }
        ...
        mAdapter.setOnItemClickListener(new BaseAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                Intent intent=new Intent(UiUtils.getContext(), ZhiHuDetailActivity.class);
                intent.putExtra(Constant.ZHIHU_IMAGE,mStoriesEntities.get(position).getImages().get(0));
                intent.putExtra(Constant.ZHIHU_ID,mStoriesEntities.get(position).getId());
                intent.putExtra(Constant.ZHIHU_TITLE,mStoriesEntities.get(position).getTitle());
    ...
    protected LoadingPage.ResultState onLoad() {
        if (mZhiHuProtocol==null){
            mZhiHuProtocol=new ZhiHuProtocol();
        }
        System.out.println(year+"~"+(month+1)+"~"+day);
        System.out.println(ZhiHuApiConstants.getZhihuApi(year,getMonthOrDay(month+1),getMonthOrDay(day)));
        return  CheckData(mZhiHuProtocol.getData(ZhiHuApiConstants.getZhihuApi(year,getMonthOrDay(month+1),getMonthOrDay(day)),2));
    }
    @Override
    ...
    protected void RefreshDatas() {
        ...
        getDataFromNet(ZhiHuApiConstants.getZhihuApi(year,getMonthOrDay(month+1),getMonthOrDay(day)));
    }
    private void getDataFromNet(final String zhiHuUrl) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                List<StoriesEntity> data = mZhiHuProtocol.getData(zhiHuUrl, 2);
    ...
    private String getMonthOrDay(int monthOrDay){
        return monthOrDay<10?0+""+monthOrDay:String.valueOf(monthOrDay);
    }
}
